...
public class DataChannelSender : MonoBehaviour
{
    ...
    public void InitClient(string serverIp, int serverPort)
    {
        ...
        ws = new WebSocket($"ws://{serverIp}:{serverPort}/{nameof(DataChannelService)}");
        ...
        dataChannel.OnOpen = () =>
        {
            Debug.Log("Sender opened a channel");
        };
        dataChannel.OnClose = () =>
        {
            Debug.Log("Sender closed a channel");
        };
